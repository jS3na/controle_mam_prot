    ...
    <div class="span12" style="margin-left: 0">
        <?php if ($this->permission->checkPermission($this->session->userdata('permissao'), 'aCliente')) { ?>
            <div class="topActions">
                ...
                -->
            <?php } ?>
            <form method="get" action="<?= base_url() ?>index.php/clientes" style="display: flex; justify-content: flex-end; gap: 10px;">
                <input type="text" name="pesquisa" id="pesquisa" placeholder="Buscar por Nome, Doc ou Cidade..." class="form-control" value="<?= $this->input->get('pesquisa') ?>">
                <select name="status" id="status" style="width: 100%;">
                    <option value="">Todos os Status</option>
                    <option value="INSTALADO" <?= $this->input->get('status') == 'INSTALADO' ? 'selected' : '' ?>>INSTALADO</option>
                    <option value="NÃO INSTALADO" <?= $this->input->get('status') == 'NÃO INSTALADO' ? 'selected' : '' ?>>NÃO INSTALADO</option>
                    <option value="FECHADO" <?= $this->input->get('status') == 'FECHADO' ? 'selected' : '' ?>>FECHADO</option>
                    <option value="COTADO" <?= $this->input->get('status') == 'COTADO' ? 'selected' : '' ?>>COTADO</option>
                    <option value="EM ANÁLISE" <?= $this->input->get('status') == 'EM ANÁLISE' ? 'selected' : '' ?>>EM ANÁLISE</option>
                    <option value="AGUARDANDO COTAÇÃO" <?= $this->input->get('status') == 'AGUARDANDO COTAÇÃO' ? 'selected' : '' ?>>AGUARDANDO COTAÇÃO</option>
                    <option value="SEM RETORNO" <?= $this->input->get('status') == 'SEM RETORNO' ? 'selected' : '' ?>>SEM RETORNO</option>
                    <option value="EM NEGOCIAÇÃO" <?= $this->input->get('status') == 'EM NEGOCIAÇÃO' ? 'selected' : '' ?>>EM NEGOCIAÇÃO</option>
                    <option value="VIABILIZADO" <?= $this->input->get('status') == 'VIABILIZADO' ? 'selected' : '' ?>>VIABILIZADO</option>
                    ...
                    <option value="VALOR ALTO" <?= $this->input->get('status') == 'VALOR ALTO' ? 'selected' : '' ?>>VALOR ALTO</option>
                    <option value="COTANDO" <?= $this->input->get('status') == 'COTANDO' ? 'selected' : '' ?>>COTANDO</option>
                </select>
                <div class="span1">
