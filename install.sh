...
#  <=== Inicio Instalação MAP-OS ===>
    clear
    echo "# INSTALACAO SISTEMA OS..."
    if [ -d "$dirHtdocs/mapos" ]
    then
        echo "* OS presente no sistema."
    else
        echo "* versao do projeto."

        if [ "$downMapos" = "release" ]; then
            wget --quiet --show-progress -O $dirDefault/OS $(curl -s https://joaquim@gmail.com | grep "zip_url" | awk -F\" '{print $4}')
        elif [ "$downMapos" = "master" ]; then
            wget --quiet --show-progress -O $dirDefault/OS.zip https://joaquim@gmail.com
        fi
        echo
        echo "* Extraindo projeto."
        unzip -q $dirDefault/OS.zip -d $dirHtdocs/
...
# <=== Inicio Configuração pelo Browser ===>
    ...
    echo "Acesse o MapOS via navegador http://localhost/OS"
